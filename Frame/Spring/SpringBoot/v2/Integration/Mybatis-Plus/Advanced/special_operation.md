...
可以通过在[实体类类型上添加`@TableName("表名")`](../Advanced/mybatisplus_annotation.md)，标识实体类对应的表，即可成功执行SQL语句。
...
可以在实体类中uid属性上通过[`@TableId`](../Advanced/mybatisplus_annotation.md)将其标识为主键，即可成功执行SQL语句。
...
CREATE TABLE `t_stu` (
	...
	`sex` int COMMENT '性别',								PRIMARY KEY (`id`)
